#This program screens a given assembly or assembly for contamination based on two #reference databases supplied by the user as fasta files, parameter 1 and parameter 2. #Parameter 1 should include sequences that are similar to those of the targeted assembly. #Parameter 2 should include sequences of known or suspected contaminants. These files #are merged and used to generate a BLAST database which is run on the target assembly. #The best hit for each contig in the assembly is chosen based on e-value score comparison. #E-values are compared based on order of magnitude and the sensitivity of this comparison i#s decided by the user. Parameter 3 should be an integer and corresponds to how much #better a “contam” hit must be than the best “good” hit to be considered contaminated. #Example: If parameter 3 is input as 5 and the best “good” hit for Contig1 is 1e-10 and the #best “contam” hit is 1e-16, Contig1 is returned as “contaminated”. If the best “contam” #hit were 1e-13, Contig1 would be returned as “suspect” for manual evaluation. Parameter #4 is similar except it is how much better a “good” hit must be than a “contam” hit. Three #files are generated as output: TAXON-good.fasta, TAXON-contam.fasta, TAXON-#suspect.fasta. BLAST output files can be found in the misc_intermediate_files directory.
...
#Writes both good.fasta and contaminated.fasta to an all.fasta #and generates a blast database since e-values are not #informative across multiple blast searches.
...
#Performs blast search on assembly using the ALL database that #includes both good and contam sequences, and formats output to #a table. 
...
#The following nested if statements compares the e-values of #hits to the same contig in the targeted assembly. Contaminant #or true sequences are determined based on the e-value of the #hit. If the hit is within a threshold determined by the end #user, it is instead output to a file of suspect sequences.
if [ "$lastLine" == "" ]
then
    lastLine=$thisLine
...
#Writes out the name of each contig to either good or contam #headers files depending on the comparisons from the above if #statement.
if [ "$perfect" != "" ]
then        
        echo "$perfectLine" >> $perfect"_headers.txt"
elif  $suspect
